...
class ArchiveHost {
    ...
    public static void main(String[] args) {
        try {
            ...
            LocalFileProvider.registerMissingFileHandler(new MissingFileHandler() {
                @Nullable
                @Override
            ...
            try {
                @NotNull HttpServer http = new HttpServer();
                http.addAll(files);
            ...
            try {
                @NotNull FtpServer ftp = new FtpServer();
                ftp.addAll(files);
                new Thread(ftp).start();
            } catch (IOException ex) {
                LOG.log(Level.SEVERE, null, ex);
            }
            @NotNull FuseServer fuse = new FuseServer("test");
            fuse.addAll(files);
